...
def identify_trend(asset):
    ...
    """
    data = fetch_historical_data(asset, 5, 30)  # Fetch 30 candles of 5 minutes each
...
def is_sideways_market(data):
    ...
    if amplitude < (data["close"].mean() * 0.01):  # Example threshold: 1% of the average close price
        return True

    # Criteria 2: Well-Defined Support and Resistance Lines
    support = data["low"].min()
    resistance = data["high"].max()
    if (data["close"] > support).sum() > (len(data) * 0.9) and (data["close"] < resistance).sum() > (len(data) * 0.9):
        return True

    # Criteria 3: Trend Indicators Show Sideways Movement
    ma20 = ta.sma(data["close"], length=20)
    ma50 = ta.sma(data["close"], length=50)
    if ma20.iloc[-1] == ma20.iloc[-2] and ma50.iloc[-1] == ma50.iloc[-2]:
        return True
    ...
    if (data["close"] - data["open"]).abs().mean() < (data["high"] - data["low"]).mean() * 0.1:  # Example threshold
        return True
    ...
    zigzag = (data["close"].diff().abs() < (data["close"].mean() * 0.005)).sum() > (len(data) * 0.8)  # Example threshold
    if zigzag:
        return True

    # Criteria 7: Narrow Bollinger Bands
    bbands = ta.bbands(data["close"], length=20)
    if (bbands["BBU_20_2.0"] - bbands["BBL_20_2.0"]).mean() < (data["close"].mean() * 0.05):  # Example threshold
        return True
