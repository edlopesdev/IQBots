...
def connect_to_iq_option(email, password):
    ...
    try:
        ...
        if check:
            ...
            print_account_balance()  # Exibir saldo após conexão bem-sucedida
            update_session_profit()  # Atualizar lucro da sessão
        else:
        ...
        if iq is None or not iq.check_connect():
            ...
            try:
                ...
                if check:
                    ...
                    set_amount()  # Certifique-se de que a função está definida antes de ser chamada
...
def analyze_indicators(asset):
    ...
    try:
        ...
        available_indicators = {
            ...
            "macd": lambda: pyti_macd(data["close"].tolist(), 12, 26),  # Fixed argument count
            ...
            "willr": lambda: pyti_willr(data["close"].tolist()),  # Fixed argument count
        ...
        for key, func in available_indicators.items():
            try:
                ...
                indicators[key] = None  # Ignore invalid indicators

        decisions = []

        if indicators.get("rsi") is not None:
            if indicators["rsi"][-1] < 30:  # Use the last value explicitly
        ...
        if indicators.get("sma") is not None:
            if data["close"].iloc[-1] > indicators["sma"][-1]:
                decisions.append("buy")
            else:
                decisions.append("sell")

        if indicators.get("ema") is not None:
            if data["close"].iloc[-1] > indicators["ema"][-1]:
                decisions.append("buy")
            else:
                decisions.append("sell")

        if indicators.get("bollinger") is not None:
            if data["close"].iloc[-1] < indicators["bollinger"][-1]:
                decisions.append("buy")
            elif data["close"].iloc[-1] > indicators["bollinger"][-1]:
                decisions.append("sell")
        ...
        if buy_votes > (total_votes / 2) + 2:
            return "buy"
        elif sell_votes > (total_votes / 2) + 2:
            return "sell"
        else:
...
def set_amount():
    ...
    global current_amount
    if iq is None or not iq.check_connect():
        log_message("Erro: API desconectada. Não é possível definir o valor inicial.")
        return  # Retorna sem executar o restante da função

    try:
        balance = iq.get_balance()
        initial_amount = balance * 0.025  # Define como 2.5% do saldo
        current_amount = initial_amount
        log_message(f"Valor inicial definido como 2.5% do saldo: R${initial_amount:.2f}")
        log_message(f"Saldo: R${balance:.2f}")
    except Exception as e:
        log_message(f"Erro ao definir o valor inicial: {e}")
