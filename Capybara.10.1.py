...
def connect_to_iq_option(email, password):
    ...
    check, reason = False, None  # Inicializar variáveis para evitar UnboundLocalError
    try:
...
from pyti import keltner_channel_upper as pyti_keltner_upper, keltner_channel_lower as pyti_keltner_lower
...
def analyze_indicators(asset):
    ...
    try:
        ...
        available_indicators = {
            "rsi": lambda: pyti_rsi(data["close"].tolist(), 14),
            "macd": lambda: pyti_macd(data["close"].tolist(), 12, 26, 9),
            "sma": lambda: pyti_sma(data["close"].tolist(), 20),
            "ema": lambda: pyti_ema(data["close"].tolist(), 9),
            "bollinger": lambda: pyti_bollinger(data["close"].tolist(), 20),
            "atr": lambda: pyti_atr(data["close"].tolist(), 14),
            "stochastic": lambda: pyti_stochastic(data["close"].tolist(), 14),
            "willr": lambda: pyti_willr(data["close"].tolist(), 14),
            "cci": lambda: pyti_cci(data["high"].tolist(), data["low"].tolist(), data["close"].tolist(), 20),
            "roc": lambda: pyti_roc(data["close"].tolist(), 12),
            "obv": lambda: pyti_obv(data["close"].tolist(), data["volume"].tolist()),
            "mfi": lambda: pyti_mfi(data["high"].tolist(), data["low"].tolist(), data["close"].tolist(), data["volume"].tolist(), 14),
            "vbt_rsi": lambda: vbt.IndicatorFactory.from_pandas_ta('rsi').run(data["close"], window=14).rsi.iloc[-1],
            "vbt_macd": lambda: vbt.IndicatorFactory.from_pandas_ta('macd').run(data["close"]).macd.iloc[-1],
            "qs_sharpe": lambda: qs.stats.sharpe(data["close"]),
            "ta_rsi": lambda: ta.momentum.RSIIndicator(data["close"], window=14).rsi().iloc[-1],
            "ta_macd": lambda: ta.trend.MACD(data["close"]).macd_diff().iloc[-1],
            "finta_sma": lambda: TA.SMA(data, 20).iloc[-1],
            "finta_ema": lambda: TA.EMA(data, 9).iloc[-1],
            "btalib_rsi": lambda: btalib.rsi(data["close"], period=14).df.iloc[-1]["rsi"],
            "btalib_macd": lambda: btalib.macd(data["close"]).df.iloc[-1]["macd"],
            "adf_test": lambda: adfuller(data["close"])[1],  # P-value of the ADF test
            "keltner_upper": lambda: pyti_keltner_upper(data["high"].tolist(), data["low"].tolist(), data["close"].tolist(), 20),
            "keltner_lower": lambda: pyti_keltner_lower(data["high"].tolist(), data["low"].tolist(), data["close"].tolist(), 20),
            "ta_atr": lambda: AverageTrueRange(data["high"], data["low"], data["close"], window=14).average_true_range().iloc[-1],
            "ta_adx": lambda: ADXIndicator(data["high"], data["low"], data["close"], window=14).adx().iloc[-1],
            "ta_stochastic": lambda: StochasticOscillator(data["high"], data["low"], data["close"], window=14).stoch().iloc[-1],
            "ta_obv": lambda: OnBalanceVolumeIndicator(data["close"], data["volume"]).on_balance_volume().iloc[-1],
        }
        ...
        if indicators.get("bollinger") is not None:
            if data["close"].iloc[-1] < indicators["bollinger"]:
                decisions.append("buy")
            elif data["close"].iloc[-1] > indicators["bollinger"]:
                decisions.append("sell")
        ...
        if indicators.get("adf_test") is not None:
            if indicators["adf_test"] < 0.05:  # Stationary series
                decisions.append("buy")
            else:
                decisions.append("sell")
        ...
        log_message(f"Votação de indicadores para {asset}: BUY={buy_votes}, SELL={sell_votes}, TOTAL={total_votes}")
        log_message(f"Decisões: {decisions}")

        if buy_votes > (total_votes / 2) + 1:
            return "buy"
        elif sell_votes > (total_votes / 2) + 1:
            return "sell"
        else:
...
def monitor_trade(trade_id, asset):
    ...
    global running  # Adicionado para parar a execução
    ...
    try:
        ...
        if result <= 0:
            ...
            if consecutive_losses >= 3:  # Verifica se houve 3 perdas consecutivas
                log_message("Muitos erros seguidos. Interrompendo execução")
                print("Muitos erros seguidos. Interrompendo execução")
                running = False  # Para a execução
                return
        ...
        # Atualizar o valor da negociação ao fim da operação
        set_amount()
...
def set_amount():
    ...
    global current_amount
    balance = iq.get_balance()
    initial_amount = balance * 0.025  # Set to 2.5% of the balance
