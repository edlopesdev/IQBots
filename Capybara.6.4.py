...
previous_amount = 2  # Armazenar o valor da negociação anterior
...
def connect_to_iq_option(email, password):
    ...
    try:
        ...
        if check:
            ...
            root.after(0, set_amount)  # Definir o valor inicial da negociação
            root.after(0, print_account_balance)  # Print account balance after successful connection
            root.after(0, update_session_profit)  # Update session profit after successful connection
        else:
...
def reconnect_if_needed():
    ...
    if iq is None or not iq.check_connect():
        ...
        if not iq or not iq.check_connect():
            ...
            root.after(0, lambda: setattr(running, False))
...
def monitor_trade(trade_id, asset):
    ...
    try:
        ...
        if result <= 0:
            ...
            if should_abandon_trade(result, current_price, MAX_LOSS):
                log_message("Abandonando negociação devido a critérios definidos.")
                current_amount = previous_amount  # Use the previous trade amount after abandonment
            else:
                if consecutive_losses >= 3:
                    ...
                    if decision == "buy":
                        ...
                        log_message(f"Reversed action to {action} for {asset}.")
                    elif decision == "sell":
                        ...
                        log_message(f"Pulando negociação para {asset}. Sem consenso.")
                        return
                previous_amount = current_amount  # Store the current amount before doubling
                current_amount = current_amount * 2  # Double the amount for the next trade
    ...
    for trade_id in trade_list.copy():
        try:
            ...
            if result is not None:
                ...
                if result <= 0:
                    consecutive_losses += 1
                    if should_abandon_trade(result, iq.get_candles(asset, 60, 1, time.time())[0]['close'], MAX_LOSS):
                        log_message("Abandonando negociação devido a critérios definidos.")
                        current_amount = previous_amount  # Use the previous trade amount after abandonment
                    else:
                        if consecutive_losses >= 3:
...
def update_session_profit():
    ...
    root.after(0, lambda: profit_label.config(text=f"Lucro: R${total_profit:.2f}", fg="green" if total_profit >= 0 else "red"))
...
def start_trading():
    ...
    if not running:
        running = True
        root.after(0, lambda: icon_label.config(image=rotating_icon))
...
def stop_trading():
    ...
    if 'static_icon' in globals():
        root.after(0, lambda: icon_label.config(image=static_icon))
    log_message("Smart Stop ativado. Parando execução após reset para valor inicial.")
...
def update_log():
    try:
        with open(log_file, "r") as file:
            ...
            root.after(0, lambda: log_text.insert(tk.END, "".join(lines[-10:])))
    except FileNotFoundError:
        root.after(0, lambda: log_text.delete(1.0, tk.END))
        root.after(0, lambda: log_text.insert(tk.END, "Log file not found. Waiting for new entries...\n"))
    if running:
...
# GUI Configuration
root = tk.Tk()
root.title("Capybara Trader v6.3")
