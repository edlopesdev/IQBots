...
# Definir variáveis globais para três slots de operação
running = False
icon_label = None
initial_amount_a = 0
initial_amount_b = 0
...
saldo_entrada = 0
saldo_saida = 0
amount_doubled_a = False
amount_doubled_b = False
amount_doubled_c = False
...
def connect_to_iq_option(email, password):
    ...
    check, reason = False, "Unknown error"  # Initialize variables with default values
    try:
        ...
        if check:
            ...
            update_balance()  # Update balance after successful connection
        else:
        ...
        if iq is None or not iq.check_connect():
            ...
            try:
                ...
                if check:
                    ...
                    update_session_profit()  # Update session profit after successful connection
                    update_balance()  # Update balance after successful connection
                    retry_attempts = 0  # Reset retry attempts after successful connection
                else:
            ...
            if not check:
                ...
                if retry_attempts > max_retries:
                    logging.error("Número máximo de tentativas de reconexão atingido. Parando tentativas.")
                    break
                delay = base_delay * (2 ** (retry_attempts - 1))  # Exponential backoff
                logging.info(f"Aguardando {delay} segundos antes de tentar reconectar novamente...")
                time.sleep(delay)
        else:
            retry_attempts = 0  # Reset retry attempts if connection is successful
        time.sleep(5)
...
def analyze_indicators(asset):
    ...
    try:
        ...
        if consecutive_losses_a >= 1:  # Invert votes starting from the second Martingale
...
def countdown(seconds):
    while seconds > 0:
        ...
        if simultaneous_trades < max_simultaneous_trades:
            log_message("Espaço disponível na fila de trades. Interrompendo contagem regressiva.")
            break
...
def execute_trades():
    global running
    global current_amount_a, current_amount_b, current_amount_c
    global consecutive_losses_a, consecutive_losses_b, consecutive_losses_c
    ...
    while running:
        ...
        for asset in assets:
            ...
            if simultaneous_trades >= max_simultaneous_trades:
                log_message("Aguardando a conclusão das negociações atuais...")
                countdown(30)
                check_trade_results()
            ...
            # Determine which slot to use for the trade
            slot = None
            if current_amount_a == initial_amount_a:
                slot = 'a'
                amount = current_amount_a
            elif current_amount_b == initial_amount_b:
                slot = 'b'
                amount = current_amount_b
            elif current_amount_c == initial_amount_c:
                slot = 'c'
                amount = current_amount_c
            else:
                while True:
                    log_message("Todos os slots estão ocupados. Aguardando a conclusão das negociações atuais...")
                    countdown(30)
                    check_trade_results()
                    update_session_profit()
                    if current_amount_a == initial_amount_a:
            ...
            log_message(f"Tentando realizar negociação: Ativo={asset}, Ação={action}, Valor = R${amount}, Slot={slot}")
            ...
            try:
                success, trade_id = iq.buy(amount, asset, action, 5)  # Negociações de 5 minutos
                if success:
                    ...
                    threading.Thread(target=monitor_trade, args=(trade_id, asset, slot)).start()
                else:
...
def switch_variables(slot):
    ...
    if slot == 'a':
        ...
        current_amount = current_amount_c

def monitor_trade(trade_id, asset, slot):
    global simultaneous_trades
    global session_profit
    global consecutive_losses_a, consecutive_losses_b, consecutive_losses_c
    global current_amount_a, current_amount_b, current_amount_c
    global saldo_saida
    global amount_doubled_a, amount_doubled_b, amount_doubled_c

    switch_variables(slot)  # Call the switch_variables function
    ...
    try:
        ...
        switch_variables(slot)  # Call the switch_variables function
        fetch_sorted_assets()  # Fetch sorted assets to ensure the asset is still valid
        analyze_last_candles(fetch_historical_data(asset, 1, 5))  # Analyze the last 5 candles for the asset
        analyze_trend(fetch_historical_data(asset, 1, 15))  # Analyze the trend for the asset
        analyze_indicators(asset)  # Analyze the indicators for the asset
        execute_trades()  # Execute trades while monitoring the current trade
        result = None
        ...
        if slot == 'a':
            if result <= 0:
                ...
                current_amount_a = current_amount_a * 2  # Correctly double the amount for the next trade
                amount_doubled_a = True  # Set the flag to indicate the amount has been doubled
                log_message(f"Dobrar valor da negociação. Próximo valor de negociação: R${current_amount_a}")
            else:
                consecutive_losses_a = 0
                set_amount()  # Reset to 5% of the balance after a win
                amount_doubled_a = False  # Reset the flag
                log_message(f"Negociação bem-sucedida. Valor de negociação resetado para: R${current_amount_a}")
        elif slot == 'b':
            if result <= 0:
                consecutive_losses_b += 1
                current_amount_b = current_amount_b * 2  # Correctly double the amount for the next trade
                amount_doubled_b = True  # Set the flag to indicate the amount has been doubled
                log_message(f"Dobrar valor da negociação. Próximo valor de negociação: R${current_amount_b}")
            else:
                consecutive_losses_b = 0
                set_amount()  # Reset to 5% of the balance after a win
                amount_doubled_b = False  # Reset the flag
                log_message(f"Negociação bem-sucedida. Valor de negociação resetado para: R${current_amount_b}")
        elif slot == 'c':
            if result <= 0:
                consecutive_losses_c += 1
                current_amount_c = current_amount_c * 2  # Correctly double the amount for the next trade
                amount_doubled_c = True  # Set the flag to indicate the amount has been doubled
                log_message(f"Dobrar valor da negociação. Próximo valor de negociação: R${current_amount_c}")
            else:
                consecutive_losses_c = 0
                set_amount()  # Reset to 5% of the balance after a win
                amount_doubled_c = False  # Reset the flag
                log_message(f"Negociação bem-sucedida. Valor de negociação resetado para: R${current_amount_c}")

        print_abc_queue()  # Print the abc queue after each trade result
    ...
    for trade_id in trade_list.copy():
        try:
            ...
            if result is not None:
                ...
                print_abc_queue()  # Print the abc queue after checking trade results
        except Exception as e:
            print(f"Erro ao verificar status da negociação {trade_id}: {e}")
            log_message(f"Erro ao verificar status da negociação {trade_id}: {e}")
...
def set_amount():
    global initial_amount_a, initial_amount_b, initial_amount_c
    global current_amount_a, current_amount_b, current_amount_c
    if iq is not None:
        balance = iq.get_balance()
        initial_amount_a = balance * 0.05  # Set to 5% of the balance
        initial_amount_b = balance * 0.05  # Set to 5% of the balance
        initial_amount_c = balance * 0.05  # Set to 5% of the balance
        current_amount_a = initial_amount_a
        current_amount_b = initial_amount_b
        current_amount_c = initial_amount_c
        log_message(f"Initial amounts set to 5% of balance: R${initial_amount_a:.2f}, R${initial_amount_b:.2f}, R${initial_amount_c:.2f}")
        update_balance()
        print_abc_queue()
    else:
        log_message("IQ Option API is not connected. Cannot set amount.")
...
def simulate_trade(success, slot):
    global current_amount_a, current_amount_b, current_amount_c
    global consecutive_losses_a, consecutive_losses_b, consecutive_losses_c
    global amount_doubled_a, amount_doubled_b, amount_doubled_c
    ...
    if slot == 'a':
        if success:
            log_message(f"Trade successful. Amount: ${current_amount_a}")
            consecutive_losses_a = 0
            current_amount_a = initial_amount_a
            amount_doubled_a = False
        else:
            log_message(f"Trade failed. Amount: ${current_amount_a}")
            consecutive_losses_a += 1
            if consecutive_losses_a <= martingale_limit:
                current_amount_a *= 2
                amount_doubled_a = True
            else:
        ...
        if success:
            log_message(f"Trade successful. Amount: ${current_amount_b}")
            consecutive_losses_b = 0
            current_amount_b = initial_amount_b
            amount_doubled_b = False
        else:
            log_message(f"Trade failed. Amount: ${current_amount_b}")
            consecutive_losses_b += 1
            if consecutive_losses_b <= martingale_limit:
                current_amount_b *= 2
                amount_doubled_b = True
            else:
                log_message("Martingale limit reached. Resetting to initial amount.")
                consecutive_losses_b = 0
                current_amount_b = initial_amount_b
        ...
        if success:
            log_message(f"Trade successful. Amount: ${current_amount_c}")
            consecutive_losses_c = 0
            current_amount_c = initial_amount_c
            amount_doubled_c = False
        else:
            log_message(f"Trade failed. Amount: ${current_amount_c}")
            consecutive_losses_c += 1
            if consecutive_losses_c <= martingale_limit:
                current_amount_c *= 2
                amount_doubled_c = True
            else:
                log_message("Martingale limit reached. Resetting to initial amount.")
                consecutive_losses_c = 0
                current_amount_c = initial_amount_c
...
def test_martingale_logic():
    ...
    simulate_trade(False, 'a')  # Falha
    assert current_amount_a == initial_amount_a * 2, f"Expected {initial_amount_a * 2}, got {current_amount_a}"
    assert amount_doubled_a == True, "Expected amount_doubled_a to be True"

    simulate_trade(False, 'a')  # Falha
    assert current_amount_a == initial_amount_a * 4, f"Expected {initial_amount_a * 4}, got {current_amount_a}"
    assert amount_doubled_a == True, "Expected amount_doubled_a to be True"

    simulate_trade(True, 'a')  # Sucesso
    assert current_amount_a == initial_amount_a, f"Expected {initial_amount_a}, got {current_amount_a}"
    assert amount_doubled_a == False, "Expected amount_doubled_a to be False"

    simulate_trade(False, 'b')  # Falha
    assert current_amount_b == initial_amount_b * 2, f"Expected {initial_amount_b * 2}, got {current_amount_b}"
    assert amount_doubled_b == True, "Expected amount_doubled_b to be True"

    simulate_trade(False, 'b')  # Falha
    assert current_amount_b == initial_amount_b * 4, f"Expected {initial_amount_b * 4}, got {current_amount_b}"
    assert amount_doubled_b == True, "Expected amount_doubled_b to be True"

    simulate_trade(True, 'b')  # Sucesso
    assert current_amount_b == initial_amount_b, f"Expected {initial_amount_b}, got {current_amount_b}"
    assert amount_doubled_b == False, "Expected amount_doubled_b to be False"

    simulate_trade(False, 'c')  # Falha
    assert current_amount_c == initial_amount_c * 2, f"Expected {initial_amount_c * 2}, got {current_amount_c}"
    assert amount_doubled_c == True, "Expected amount_doubled_c to be True"

    simulate_trade(False, 'c')  # Falha
    assert current_amount_c == initial_amount_c * 4, f"Expected {initial_amount_c * 4}, got {current_amount_c}"
    assert amount_doubled_c == True, "Expected amount_doubled_c to be True"

    simulate_trade(True, 'c')  # Sucesso
    assert current_amount_c == initial_amount_c, f"Expected {initial_amount_c}, got {current_amount_c}"
    assert amount_doubled_c == False, "Expected amount_doubled_c to be False"
...
def update_balance():
    if iq is not None:
        ...
        log_message(f"Updated balance: R${balance:.2f}")
    else:
        log_message("IQ Option API is not connected. Cannot update balance.")

def print_abc_queue():
    log_message(f"Queue A: {current_amount_a}, Queue B: {current_amount_b}, Queue C: {current_amount_c}")
...
# GUI Configuration
root = tk.Tk()
root.title("Capybara Trader v6.7")
