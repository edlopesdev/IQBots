...
def fetch_candles(asset, period=1, count=60):
    """Obtém os candles para o ativo especificado e trata colunas ausentes."""
    try:
        candles = iq.get_candles(asset, period * 60, count, time.time())
        if not candles:
...
def store_trade_data_and_update_model(features, target, result):
    """Salva os dados da negociação no CSV e atualiza o modelo apenas se for um win."""
    print("Salvando dados da negociação vencedora...")
    store_trade_data(features, target, result)
    print("Verificando se o modelo precisa ser atualizado...")
    if result > 0:
        success = train_and_save_model()
        if success:
            print("Modelo atualizado com sucesso!")
        else:
            print("Falha ao atualizar o modelo.")
...
def execute_trades():
    ...
    while running:
        ...
        for asset in assets:
            ...
            features = candles.values.flatten()
