...
# Arquivo: Capybara_NoZZ.v.0.8.py
#PECUNIA IMPROMPTA
...
def analyze_trend(data):
    """
    Analyzes the trend of the last 10 candles.
    ...
    close_mean = data["close"].iloc[-10:].mean()
    open_mean = data["open"].iloc[-10:].mean()
    close_last = data["close"].iloc[-1]
    open_last = data["open"].iloc[-1]

    log_message(f"Analisando tendência: close_mean={close_mean}, open_mean={open_mean}, close_last={close_last}, open_last={open_last}")

    # Identify specific patterns
    if data["close"].iloc[-3] > data["close"].iloc[-2] and data["close"].iloc[-2] < (data["close"].iloc[-3] + data["close"].iloc[-1]) / 2 and data["close"].iloc[-1] < data["close"].iloc[-3]:
        return "down"
    elif data["close"].iloc[-3] < data["close"].iloc[-2] and data["close"].iloc[-2] > (data["close"].iloc[-3] + data["close"].iloc[-1]) / 2 and data["close"].iloc[-1] > data["close"].iloc[-3]:
        return "up"
    elif close_mean > open_mean and close_last > open_last:
        return "up"
    elif close_mean < open_mean and close_last < open_last:
        return "down"
    elif abs(close_last - open_last) > (close_mean * 0.005):  # Adjusted sensitivity to 0.5%
        return "up" if close_last > open_last else "down"
    else:
        return "neutral"
...
def analyze_last_candles(data):
    ...
    close_last_5 = data["close"].iloc[-5:].mean()
    open_last_5 = data["open"].iloc[-5:].mean()

    if close_last_5 > open_last_5:
        return "up"
    elif close_last_5 < open_last_5:
        return "down"
    else:
        return "neutral"
...
def identify_trend(asset):
    ...
    """
    data = fetch_historical_data(asset, 1, 10)  # Fetch 10 candles of 1 minute each
    trend = analyze_trend(data)
    print(f"Tendência para {asset}: {trend}")
    return trend
    ...
    try:
        ...
        for key, func in available_indicators.items():
            try:
                with warnings.catch_warnings():
                    ...
                    result = func()
                    if result is not None and isinstance(result, pd.Series) and not result.empty:
                        indicators[key] = result.iloc[-1]
                        log_message(f"Indicador {key.upper()} para {asset}: {indicators[key]}")
        ...
        if indicators.get("trend") is not None:
            if indicators["trend"] == "up":
                ...
                decisions.append("sell")
            print(f"Tendência para {asset}: {indicators['trend']}")
        ...
        if trend == "up" and sell_votes > buy_votes:
            log_message(f"Tendência de alta para {asset}. Ignorando sinais de venda.")
            return None
        elif trend == "down" and buy_votes > sell_votes:
            log_message(f"Tendência de baixa para {asset}. Ignorando sinais de compra.")
            return None
...
def is_sideways_market(data):
    """
    Identifies if the market is sideways based on the given criteria.
    Returns True if the market is sideways, otherwise False.
    """
    votes = 0
    total_criteria = 8

    # Criteria 1: Small Amplitude of Oscillation
    amplitude = data["high"].max() - data["low"].min()
    if amplitude < (data["close"].mean() * 0.02):  # Adjusted threshold: 2% of the average close price
        log_message("Mercado lateralizado: Pequena amplitude de oscilação.")
        votes += 1

    # Criteria 2: Well-Defined Support and Resistance Lines
    support = data["low"].min()
    resistance = data["high"].max()
    if (data["close"] > support).sum() > (len(data) * 0.8) and (data["close"] < resistance).sum() > (len(data) * 0.8):
        log_message("Mercado lateralizado: Linhas de suporte e resistência bem definidas.")
        votes += 1

    # Criteria 3: Trend Indicators Show Sideways Movement
    ma20 = ta.sma(data["close"], length=20)
    ma50 = ta.sma(data["close"], length=50)
    ma20_last = ma20.iloc[-1] if not ma20.empty else None
    ma20_prev = ma20.iloc[-2] if len(ma20) > 1 else None
    ma50_last = ma50.iloc[-1] if not ma50.empty else None
    ma50_prev = ma50.iloc[-2] if len(ma50) > 1 else None
    if ma20_last == ma20_prev and ma50_last == ma50_prev:
        log_message("Mercado lateralizado: Indicadores de tendência mostram movimento lateral.")
        votes += 1
    rsi = ta.rsi(data["close"], length=14)
    rsi_last = rsi.iloc[-1] if not rsi.empty else None
    if rsi_last and 45 < rsi_last < 55:
        log_message("Mercado lateralizado: RSI entre 45 e 55.")
        votes += 1

    # Criteria 4: Absence of Strong Directional Candles
    if (data["close"] - data["open"]).abs().mean() < (data["high"] - data["low"]).mean() * 0.2:  # Adjusted threshold
        log_message("Mercado lateralizado: Ausência de velas direcionais fortes.")
        votes += 1

    # Criteria 5: Low Trading Volume
    if data["volume"].mean() < data["volume"].rolling(window=20).mean().iloc[-1] * 0.5:  # Example threshold
        log_message("Mercado lateralizado: Baixo volume de negociação.")
        votes += 1

    # Criteria 6: Zigzag Movements
    zigzag = (data["close"].diff().abs() < (data["close"].mean() * 0.01)).sum() > (len(data) * 0.8)  # Adjusted threshold
    if zigzag:
        log_message("Mercado lateralizado: Movimentos em zigzag.")
        votes += 1

    # Criteria 7: Narrow Bollinger Bands
    bbands = ta.bbands(data["close"], length=20)
    if (bbands["BBU_20_2.0"] - bbands["BBL_20_2.0"]).mean() < (data["close"].mean() * 0.1):  # Adjusted threshold
        log_message("Mercado lateralizado: Bandas de Bollinger estreitas.")
        votes += 1

    # Criteria 8: Minimum Variation Between Candles
    min_variation = (data["close"] - data["open"]).abs().min()
    if min_variation < (data["close"].mean() * 0.001):  # Example threshold: 0.1% of the average close price
        log_message("Mercado lateralizado: Variação mínima entre os candles.")
        votes += 1

    # Determine if the market is sideways based on the votes
    if votes > (total_criteria / 2):
        log_message("Mercado lateralizado: Consenso de mais de 50% dos critérios.")
        return True
    else:
        log_message("Mercado não está lateralizado: Menos de 50% dos critérios atendidos.")
        return False
    ...
    while running:
        ...
        for asset in assets:
            ...
            if simultaneous_trades >= 1:  # Ensure only one trade at a time
                ...
                check_trade_results()  # Remove the 'asset' argument
...
def stop_trading():
    ...
    smart_stop = True
    running = False
    icon_label.config(image="static_icon.png")
    log_message("Smart Stop ativado. Parando execução após reset para valor inicial.")
...
# GUI Configuration
root = tk.Tk()
root.title("Capybara Trader NoZZv0.8")
