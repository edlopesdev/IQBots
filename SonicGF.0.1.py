...
trade_list_s = []
saldo_entrada_s = 0  # Saldo na hora de começar uma operação
saldo_saida_s = 0  # Saldo no fim da operação
            ...
            try:
                ...
                if check:
                    ...
                    try:
                        ...
                        logging.error("Erro ao decodificar JSON na resposta de reconexão.")
            except Exception as e:
...
# Update the analyze_indicators_s function to include the Grande Filtro indicator
...
def execute_trades_s():
    ...
    while running_s:
        ...
        for asset in assets:
            ...
            saldo_entrada_s = iq_s.get_balance()  # Store balance before trade
            print(f"Balance before opening trade: {saldo_entrada_s}")  # Display balance before trade
            try:
                ...
                if success:
                    ...
                    print(f"Balance after opening trade: {iq_s.get_balance()}")  # Display balance after opening trade
        ...
        if smart_stop_s and current_amount_s == initial_amount_s:
            running_s = False
...
def monitor_trade_s(trade_id, asset):
    ...
    try:
        ...
        saldo_saida_s = iq_s.get_balance()  # Store balance after trade
        ...
        print(f"Balance after closing trade: {iq_s.get_balance()}")  # Display balance after closing trade
...
def update_session_profit_s():
    global saldo_entrada_s, saldo_saida_s, session_profit_s
    total_profit = saldo_saida_s - saldo_entrada_s  # Calculate total profit based on balance difference
    profit_label.config(text=f"Lucro: R${total_profit:.2f}", fg="green" if total_profit >= 0 else "red")

# Update the set_amount function to use auto_initial_amount_s
...
def smart_stop_trading():
    global smart_stop_s
    smart_stop_s = True
    log_message("Smart Stop activated. Trading will stop after the current trade.")
...
# Custom logging handler to redirect logs to the ScrolledText widget
...
# GUI Configuration
root = tk.Tk()
root.title("Sonic Trader GFv0.1")
...
smart_stop_button = tk.Button(root, text="Smart Stop", command=smart_stop_trading, bg="#F44336", fg="white", font=("Helvetica", 12))
...
# Add a try-except block to handle JSONDecodeError in the WebSocket callback
def websocket_callback(message):
    try:
        # Ensure the message is a valid JSON string
        if not isinstance(message, str) or not message.strip() or message.strip()[0] not in ['{', '[']:
            logging.error(f"Invalid JSON message received: {message}")
            return
        json.loads(str(message))
    except json.JSONDecodeError as e:
        logging.error(f"JSONDecodeError in websocket_callback: {e}. Message: {message}")
        # Add the asset to the ignore list
        asset = extract_asset_from_message(message)
        if asset:
            add_asset_to_ignore_list(asset)
    except Exception as e:
        logging.error(f"Unexpected error in websocket_callback: {e}. Message: {message}")
        # Add the asset to the ignore list
        asset = extract_asset_from_message(message)
        if asset:
            add_asset_to_ignore_list(asset)
