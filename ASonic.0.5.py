...
def execute_trade(self, asset, action):
    ...
    try:
        ...
        success, trade_id = iq.place_trade(asset, action, trade_amount, duration=1)
...
class TradingLogic:
    ...
    def execute_trade(self, asset, action):
        ...
        try:
            ...
            log_message(f"Executando trade: Ativo={asset}, Ação={action}, Valor={trade_amount}")
            success, trade_id = self.api.place_trade(asset, action, trade_amount, duration=1)
...
# GUI Configuration
root = tk.Tk()
root.title("Sonic v0.5")
root.configure(bg="#111b1c")  # Set dark theme background
...
icon_label = tk.Label(root, image=static_icon, bg="#111b1c")
...
# Timer label
timer_label = Label(root, text="00:00", font=("Courier", 20), bg="#111b1c", fg="white")
timer_label.grid(row=2, column=0, columnspan=5, padx=10, pady=10)
...
switch_account_button = tk.Button(root, text="Switch Account", command=switch_account_type, bg="#2196F3", fg="white", font=("Helvetica", 10))
switch_account_button.grid(row=0, column=3, padx=3, pady=3)

amount_label = tk.Label(root, text="Initial Amount:", bg="#111b1c", fg="white", font=("Helvetica", 12))
...
set_button = tk.Button(root, text="Set Amount", command=lambda: set_amount(float(amount_entry.get())), bg="#FFC107", fg="white", font=("Helvetica", 12))
set_button.grid(row=1, column=3, padx=5, pady=5)


profit_label = tk.Label(root, text="Lucro: R$0.00", font=("Helvetica", 16), bg="#111b1c", fg="white")
